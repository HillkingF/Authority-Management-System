...
如果表单信息校验成功、登录成功，应该要生成一个JWT（https://blog.csdn.net/qq_37636695/article/details/79265711）中。以后每次访问时，就会携带jwt告诉后端，请求是谁发起的。
...
在src 目录下新建 `globalFun.js`， 用于全局权限控制：
...
在vue页面中需要进行权限控制的地方，使用v-if调用上面js中的方法：
...
将 `globalFun.js` 这个全局权限引入 `main.js` 中
...
Mock.mock('/sys/menu/nav', "get", () =>  {
    ...
    // 【authoritys是用户权限信息】
    let authoritys = ['sys:user:list', "sys:user:save", "sys:suer:del"]
